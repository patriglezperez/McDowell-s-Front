...
export default function Delivering(props) {
  ...
  useEffect(() => {
    if (orders) {
      ...
      const kaka = orders.every((e) => {
        return e.statuss === "delivering";
      });
  ...
  async function handleFinish() {
    try {
      ...
      await axios.patch(
        `${process.env.REACT_APP_API_URL}orders/finish/${orders.orderDay}`
      );
      // initialize
      orders = null;
      setFinish(false);
      ...
      {!orders ? null : (
        <div className="orders--kitchen cook">
          <div className="cook-top">
            ...
            {/* /// ajustar si viene un solo menu o varios */}
            {finish ? (
